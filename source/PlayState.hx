...
class PlayState extends MusicBeatState
{
	...
	public static var invisibledad:Character;
	public static var justice2:Character;
	...
	override public function create()
	{
		...
		if (SONG.song.toLowerCase() == "the-trial" && playCutscene){
		...
		switch(SONG.stage)
		{
			...
			case 'desert':
				...
				case 'hall':
				{
						...
						var throne:FlxSprite = new FlxSprite(-1270, -800).loadGraphic(Paths.image('justice/hall/throne'));
						throne.antialiasing = true;
						throne.scrollFactor.set(0.9, 0.9);
						throne.setGraphicSize(Std.int(throne.width * 0.7));
						throne.active = false;
						add(throne);

						var candles:FlxSprite = new FlxSprite(-1270, -800).loadGraphic(Paths.image('justice/hall/candles'));
						candles.antialiasing = true;
						candles.scrollFactor.set(0.9, 0.9);
						candles.setGraphicSize(Std.int(candles.width * 0.7));
		...
		if (FlxG.save.data.characterSelected != null)
			SONG.player1 = FlxG.save.data.characterSelected;
		...
		add(dad);
		if (dad.curCharacter == 'justice')
			{
				FlxTween.tween(dad, {y: dad.y - 50}, 1.7, {type: PINGPONG});

				invisibledad = new Character(dad.x, dad.y - 340, 'dad');
				invisibledad.alpha = 0;
				add(invisibledad);
				FlxTween.tween(invisibledad, {y: invisibledad.y - 50}, 1.7, {type: PINGPONG});
			}
			
		if (SONG.song.toLowerCase() == 'judgment')
			{
				justice2 = new Character(dad.x, dad.y, 'justice2');
				FlxTween.tween(justice2, {y: justice2.y - 50}, 1.7, {type: PINGPONG});
				justice2.alpha = 0;
				add(justice2);
			}
		...
		if (SONG.song.toLowerCase() == 'judgment')
			{
			  ...
			  {		  
			  	case 235: 
					FlxTween.tween(FlxG.camera, {zoom: 1.11}, 1.4, {ease: FlxEase.sineInOut});
					FlxG.camera.follow(invisibledad, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
				case 239:
					dad.playAnim('laugh', false);
				case 256:
					FlxTween.tween(FlxG.camera, {zoom: 0.67}, 0.06, {ease:FlxEase.sineIn});
					FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
				case 1281:
					dad.playAnim('pigeon', false);
				case 1286:
					gf.playAnim('Disappointment', false);
				case 1307:
				...
				case 1352:
					dad.alpha = 0;
					justice2.alpha = 1;
					justice2.playAnim('ugh', false);
			  }
			  
			}

		if (SONG.song.toLowerCase() == 'executioner')
			{
				switch(curStep)
				{
					case 2:
						FlxTween.tween(FlxG.camera, {zoom: 0.97}, 1.4, {ease: FlxEase.sineInOut});
						FlxG.camera.follow(invisibledad, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
					case 25, 288:
						FlxTween.tween(FlxG.camera, {zoom: 0.78}, 0.06, {ease:FlxEase.sineIn});
						FlxG.camera.follow(camFollow, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));		
					case 267:
						FlxTween.tween(FlxG.camera, {zoom: 1.11}, 1.4, {ease: FlxEase.sineInOut});
						FlxG.camera.follow(invisibledad, LOCKON, 0.04 * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
